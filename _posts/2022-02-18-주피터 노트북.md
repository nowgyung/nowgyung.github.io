...
주피터 노트북은 웹 브라우저 상에서 개발을 할 수 있는 도구이며, 코드를 Cell 단위로 묶어서 실행하고 그래프나 표, 이미지나 영상 등을 쉽게 볼 수 있어서 특히 데이터 관련 작업을 할 때 많이 활용됩니다.
...
```python
# cell을 marktdown으로 설정시 (단축기 M - 해제 시 Y)
# #과 적힌 부분은 제목처럼, #갯수가 늘어날수록 글자 크기가 줄어든다
# >표시 부등호가 적힌 부분은 인용구 처럼 표현된다
# 다시 편집하려면 더블 클릭
```
...
```python
# 이미지 불러오기 1) 스크린샷 + Ctrl+v  2) 이미지 저장 후  Edit + Insert image  로 불러올수 있다.
```
...
```python
#  앞 번호를 1.로 통일 해서 정렬하면 알아서 순서를 매겨준다. 
```
...
```python
# 글을 적고 띄어쓰기를 하지않고 다음줄에 링크 + ctrl+enter 할 경우 한 줄에 모든 글이 생성 / 글 다음 띄어쓰기 2번 하면 다음줄에 생성
```
...
```python
# %%HTML을 적고 다음줄에 HTML문법을 적으면 HTML문법으로 인식 웹 페이지 내에서 렌더링 /  영상의 소스코드를 복사+붙여넣기
```
...
```python
# random.randint?? , 모듈에 대한 정보가 필요할 때
```
...
```python
print(num) #소스 실행시 셀의 숫자 순서가 엉키지 않도록 주의
```
...
# for i in range(50):
#     print(i)
#     time.sleep(1) # 코드 진행 중 아직 끝나지않았을때 셀 숫자 부분에 *표시 , 중간에 멈추고 싶을 때  키보드 인터럽트 or I,I
```
...
```python
# 하이픈(-) 하나만 적으면 순서 없는 목록이 되고 세개를 적으면 얇은 줄이 생김
```
...
<table id="T_066fd_">
  ...
  <tbody>
    <tr>
      <th id="T_066fd_level0_row0" class="row_heading level0 row0" >Tumour (Positive)</th>
      <td id="T_066fd_row0_col0" class="data row0 col0" >38.000000</td>
      <td id="T_066fd_row0_col1" class="data row0 col1" >2.000000</td>
      <td id="T_066fd_row0_col2" class="data row0 col2" >18.000000</td>
      <td id="T_066fd_row0_col3" class="data row0 col3" >22.000000</td>
      <td id="T_066fd_row0_col4" class="data row0 col4" >21</td>
      <td id="T_066fd_row0_col5" class="data row0 col5" >nan</td>
    </tr>
    <tr>
      <th id="T_066fd_level0_row1" class="row_heading level0 row1" >Non-Tumour (Negative)</th>
      <td id="T_066fd_row1_col0" class="data row1 col0" >19.000000</td>
      <td id="T_066fd_row1_col1" class="data row1 col1" >439.000000</td>
      <td id="T_066fd_row1_col2" class="data row1 col2" >6.000000</td>
      <td id="T_066fd_row1_col3" class="data row1 col3" >452.000000</td>
      <td id="T_066fd_row1_col4" class="data row1 col4" >226</td>
      <td id="T_066fd_row1_col5" class="data row1 col5" >232.000000</td>
    </tr>
  </tbody>
</table>
...
```python
# 셀 번호를 정렬하고 싶다면  kERNEL + Restart & Run ALL 클릭 
```


```python
# 주피터 노트북 종료시  파일 저장 + File + Close and Halt , 명령 프롬포트 창에서 Ctrl + C + C
```
