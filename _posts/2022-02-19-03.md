...
```python
df.reset_index() # 새로운 index생성 기존의 지원번호 index는 column이 되어버림
```
...
```python
df.reset_index(drop=True) # 원래 쓰던 '지원번호' 인덱스 삭제 (실제 데이터에 반영X 보여지는지만 확인)
```
