...
df_m = pd.read_excel(file_name, skiprows=3, index_col='행정기관', usecols='B,E:Y') #원하는 coloumn
df_m.head(3)
```

<pre>
C:\Users\tkdnf\anaconda3\lib\site-packages\openpyxl\styles\stylesheet.py:226: UserWarning: Workbook contains no default style, apply openpyxl's default
...
```python
import pandas as pd
df_w = pd.read_excel(file_name, skiprows=3, index_col='행정기관', usecols='B,AB:AV') #원하는 coloumn
df_w.head(3)
```
...
plt.show() # show 를 먼저 해버리면 데이터가 의미가 없어지기에 먼저 저장 후 show
```
